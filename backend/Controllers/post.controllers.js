...
export const createPost = async (req, res) => {
    try {
        ...
        if (!text && !img) {
            return res.status(404).json({error: "Anypost should contain at least an image or text"})
        }
        ...
        if (img) {
            const uploaderResponse = await cloudinary.uploader(img)
...
export const deletePost = async (req, res) => {
    ...
    try {
        ...
        if (post.user.toString() !== req.user._id.toString()) {
            return res.status(404).json({error: "You are not allowed to delete that post"})
        }
...
export const getFollowingPosts = async (req, res) => {
    try {
        ...
        const followingPosts = await postModel.find({user: {$in : following} }).sort({createdAt: -1}).populate({
            path: "user",
            select: "-password",
        }).populate({
            path: "comments.user",
...
export const getUserPosts = async (req , res) => {
    try {
        ...
        const posts = await postModel.findOne({user: user._id}).sort({createdAt: -1}).populate({
            path: "user",
            select: "-password",
        }).populate({
            path: "comments.user",
